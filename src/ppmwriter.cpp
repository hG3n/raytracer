...
ppmwriter::write(Pixel const& p) 
{
  ...
  if (pos + ((sizeof(Color)/sizeof(Color::value_t)) - 1) > data_.size() || (int)buf_pos < 0) {
    ...
    data_[pos] = (unsigned int)std::max(0.0, std::min(255.0 * p.color[0], 255.0));
    data_[pos+1] = (unsigned int)std::max(0.0, std::min(255.0 * p.color[1], 255.0));
    data_[pos+2] = (unsigned int)std::max(0.0, std::min(255.0 * p.color[2], 255.0));
  }
}
